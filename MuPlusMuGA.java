...
// Date: 12/2/22
//
// Purpose: To have all overloaded methods of the mu plus mu
...
public class MuPlusMuGA extends GenericGA {
    ...
    public void resetPopulation() {
        ...
        bufferPopulation = calcPopulationFitness(bufferPopulation);
        
        population = orderPopulation(population);
        bufferPopulation = orderPopulation(bufferPopulation);

        while(newPopulation.size() < getPopulationSize()) {
            int i = 0;
            int j = 0;
            if(population[i].getFitness() >= bufferPopulation[j].getFitness()) {
                newPopulation.add(population[i]);
                i++;
            } else {
                newPopulation.add(bufferPopulation[j]);
                j++;
            }
        }

        for(int i = 0; i < population.length; i++) {
            population[i] = newPopulation.remove(0);
    ...
    public void printSolution() {
        System.out.println("");
        System.out.println("Mu plus Mu GA:");
        ...
        System.out.println("Mu plus Mu GA reached a solution in: " + cycles + " cycles.");
